...
- **主选(最多3只, 均分仓位)**：
  ...
  - **终值位置计算公式** = (竞价最终价 − 竞价区间低) ÷ (竞价区间高 − 竞价区间低) × 100%
    - 字段口径(stk_auction_o)：竞价最终价 = `open`(=当日开盘价/9:25最终撮合价)、区间高 = `high`、区间低 = `low`。
    - 衡量最终价在竞价自身区间内的位置(与相对前收涨跌无关)：=100 落在区间顶部(抢筹高开,剔)；=0 落在底部；`high=low` 单一价不剔。
    - 例(06-26)：德明利 open=high=881.1 → 终值位置=100(剔)；长川科技 open=low=311.0 → 终值位置=0(留)。
