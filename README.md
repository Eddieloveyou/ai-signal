# AI双引擎·量比策略 v2.2 — 每日早盘信号

云端 routine 每交易日 9:25 克隆本仓库并运行 `daily_signal.py`，输出**当天买哪几只**并推送到你的飞书/企业微信/Telegram。

## 规则 v2.2
- 全池(60只AI主池)等权指数 T-1收盘 > MA20 → **进攻**；≤ MA20(熄火) → **停泊银行ETF(512800)或黄金ETF(518880)**(停泊收益仅作参考，回测中若低于进攻主收益则不停泊)。
...
- **兜底**(主选选不出)：去掉量比∈(3,20)和20日涨幅上限(留贴高+趋势) → 量比第1，满仓1只。
